  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        <section className="relative py-24 px-4 bg-primary" style={{ backgroundImage: "url(/images/hero-bg.jpg)", backgroundSize: "cover", backgroundPosition: "center" }}>
        ...
        <div className="max-w-6xl mx-auto px-4 py-8 flex flex-wrap gap-2">
          {[1,2,3,4,5].map((i) => <div key={i} className="h-10 w-24 rounded-2xl bg-muted animate-pulse" />)}
        </div>
...
function EventsPageContent() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <main className="flex-1">
        {/* Hero – teljesen új: bg kép + új szöveg */}
        <section
        ...
        >
          ...
          <div className="relative z-10 max-w-4xl mx-auto text-center">
            ...
            <p className="text-white/90 text-lg md:text-xl mb-10">
              Keresse meg a programot, hasonlítsa össze a jegyárakat. Koncertek, fesztiválok, sport, színház – egy helyen.
            </p>
            <form onSubmit={handleSearch} className="flex gap-3 max-w-xl mx-auto" role="search" aria-label="Keresés">
        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-8 py-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm font-semibold text-foreground">Kategória</span>
          </div>
          <div className="flex flex-wrap gap-2" role="group" aria-label="Kategória">
            {categories.map((cat) => (
              <button
                key={cat}
        ...
        {/* Lista – új grid + kártya */}
        <section className="max-w-6xl mx-auto px-4 lg:px-8 py-12" aria-label="Eseménylista">
          <p className="text-muted-foreground mb-8">
            <strong className="text-foreground">{filtered.length}</strong> esemény
          </p>
          {filtered.length === 0 ? (
            <div className="text-center py-20 bg-secondary rounded-3xl border-2 border-border">
              <p className="text-muted-foreground text-lg">Nincs találat a megadott feltételekre.</p>
              <p className="text-sm text-muted-foreground mt-2">Próbáljon más kategóriát vagy keresési kifejezést.</p>
            </div>
          ) : (
